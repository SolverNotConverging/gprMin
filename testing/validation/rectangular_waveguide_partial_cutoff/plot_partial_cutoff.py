...
def read_series(
    ...
    return (
        ...
        np.asarray([bool(int(row["power_wave_valid"])) for row in rows]),
    )
...
def plot_case(root: Path) -> Path:
    ...
    path = paths[0]
    s11_frequency, _s11, s11_db, s11_valid, s11_power_valid = read_series(path, 1)
    s21_frequency, s21, s21_db, s21_valid, s21_power_valid = read_series(path, 2)
    ...
    if not np.all(s11_valid) or not np.all(s21_valid):
    ...
    for label, magnitude_db, valid, color in (
        ("gprMax S11", s11_db, s11_valid, "tab:blue"),
        ("gprMax S21", s21_db, s21_valid, "tab:orange"),
    ):
    ...
    phase_axis.plot(
        frequency_ghz[s21_valid],
        integrated_s21_phase_deg[s21_valid],
        marker="o",
        markersize=3.5,
        linewidth=1.5,
        color="tab:orange",
